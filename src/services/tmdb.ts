...
import { classifyGenres, tmdbGenre } from '../lib/recommendationEngine'
...
const genreIds:Record<string,number[]>={Crime:[80],Thriller:[80,9648,10759],Komedi:[35],Drama:[18],'Sci-fi':[10765],Fantasy:[10765],Action:[10759],Mystik:[9648],Historiskt:[10768],Dokumentärt:[99],Skräck:[9648,10765],Feelgood:[35,10751]}
const catalogCache=new Map<string,{expires:number;catalog:Catalog}>()
...
export async function fetchSeries(selected:string[],preferredGenres:string[]=[]):Promise<Catalog>{
 const demo=(notice:string):Catalog=>({series:mockSeries,demo:true,notice})
 const cacheKey=[...selected].sort().join('|')+';'+[...preferredGenres].sort().join('|')
 ...
 try {
  ...
  const matchingIds=[...new Set(preferredGenres.flatMap(g=>genreIds[g]||[]))]
  const base={watch_region:'SE',with_watch_providers:ids.join('|'),with_watch_monetization_types:'flatrate|free|ads','vote_count.gte':'20',include_adult:'false'}
  const searches:Record<string,string>[]=[{sort_by:'popularity.desc'},{sort_by:'vote_average.desc'}]
  if(matchingIds.length)searches.push({sort_by:'popularity.desc',with_genres:matchingIds.join('|')})
  const pages=await Promise.all(searches.map(params=>get<{results:Tv[]}>('/discover/tv',{...base,...params})))
  const unique=[...new Map(pages.flatMap(p=>p.results).map(p=>[p.id,p])).values()].slice(0,60)
  const results=await Promise.allSettled(unique.map(async item=>{
   ...
   const baseGenres=item.genre_ids.map(tmdbGenre).filter(Boolean)
   const profile=estimateProfile(item.id,baseGenres)
   const genres=classifyGenres(item.genre_ids,item.overview||'',profile)
   return {id:item.id,title:item.name,year:item.first_air_date?.slice(0,4)||'—',rating:item.vote_average,popularity:item.popularity,genres,providers,overview:item.overview||'Beskrivning saknas på svenska.',posterPath:item.poster_path,watchUrl:se?.link,profile} satisfies Series
  }))
  if(results.length&&results.every(r=>r.status==='rejected'))throw new Error('Tillgängligheten kunde inte kontrolleras')
  const catalog={series:results.flatMap(r=>r.status==='fulfilled'&&r.value.providers.length?[r.value]:[]),demo:false,notice:results.some(r=>r.status==='rejected')?'Vissa serier kunde inte kontrolleras. Visar verifierade förslag.':''}
  if(catalog.series.length)catalogCache.set(cacheKey,{catalog,expires:Date.now()+20*60_000})
  return catalog
 } catch {return demo('TMDB kunde inte nås. Visar demo med exempeldata, inte aktuell tillgänglighet.')}
}
export async function fetchSeriesDetails(id:number):Promise<Pick<Series,'episodeMinutes'|'seasons'>>{
 try {
  const details=await get<{episode_run_time?:number[];number_of_seasons?:number}>(`/tv/${id}`)
  return {episodeMinutes:details.episode_run_time?.find(n=>Number.isFinite(n)&&n>0),seasons:details.number_of_seasons&&details.number_of_seasons>0?details.number_of_seasons:undefined}
 } catch {return {}}
}
